...
describe Wire::Resource::NetworkInjection do
  ...
  let(:ni) { Wire::Resource::NetworkInjection.new('TEST',{ 'NETWORK_A' => { :dhcp => {}}, 'NETWORK_B' => {}}, %w(4711 4712 4713), '/tmp/wire_state') }
  let(:params) { ['NETWORK_A:NETWORK_A NETWORK_B:NETWORK_B:NODHCP', '4711 4712 4713'] }
  it 'should initialize correctly' do
    ni.to_s.should eq('NetworkInjection:[TEST,containers=4711/4712/4713,' \
        'network_args=NETWORK_A:NETWORK_A NETWORK_B:NETWORK_B:NODHCP]')
  end

  it 'should construct helper params correctly' do
    ni.construct_helper_params.should eq('NETWORK_A:NETWORK_A NETWORK_B:NETWORK_B:NODHCP')
  end
  ...
  it 'should call helper with attach action' do
    ni.stub(:with_helper).with('attach', params, '-s /tmp/wire_state').and_return(true)
    ni.up.should eq(true)
  end

  it 'should call helper with detach action' do
    ni.stub(:with_helper).with('detach', params, '-s /tmp/wire_state').and_return(true)
